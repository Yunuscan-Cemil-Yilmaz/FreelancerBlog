...
const models = [
    { name: 'InteractionRequest', kebab: 'interaction-requests', apiPrefix: 'interaction-requests', hasId: false },
...
models.forEach(model => {
    const featureDir = path.join(baseDir, model.kebab);
    const serviceFile = path.join(featureDir, 'services', `${model.kebab}.ts`);
    const listTsFile = path.join(featureDir, 'pages', 'list', 'list.ts');
    const listHtmlFile = path.join(featureDir, 'pages', 'list', 'list.html');
    const detailTsFile = path.join(featureDir, 'pages', 'detail', 'detail.ts');
    const detailHtmlFile = path.join(featureDir, 'pages', 'detail', 'detail.html');

    // Make dirs
    fs.mkdirSync(path.join(featureDir, 'services'), { recursive: true });
    fs.mkdirSync(path.join(featureDir, 'pages', 'list'), { recursive: true });
    fs.mkdirSync(path.join(featureDir, 'pages', 'detail'), { recursive: true });
...
`;
    ...
    // LIST TS
    const listTsContent = `import { Component, inject, OnInit } from '@angular/core';
...
export class List implements OnInit {
  ...
  // 4 views: 0=All, 1=Unread (is_readed=false), 2=Handled (is_handled=true), 3=Unhandled (is_handled=false)
  currentTabIndex = 0;
...
`;
    fs.writeFileSync(listTsFile, listTsContent);

    // LIST HTML
    const listHtmlContent = \`<div class="p-6">
...
\`;
    fs.writeFileSync(listHtmlFile, listHtmlContent);

    // DETAIL TS
    const fkKey = model.hasId ? model.idName : 'interaction_id';
    const detailTsContent = `import { Component, inject, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
...
export class Detail implements OnInit {
  ...
  addDetail() {
    ...
    // add foreign key dynamically based on model
    const fkKey = '${fkKey}' === 'interaction_id' && '${model.name}' !== 'InteractionRequest' ? '${model.kebab}'.replace(/-/g, '_') + '_id' : '${fkKey}';
    const actualFk = '${model.name}' === 'BlogInteractionRequest' ? 'blog_interaction_id' : ('${model.name}' === 'RepoInteractionRequest' ? 'repo_interaction_id' : 'interaction_id');
...
`;
    fs.writeFileSync(detailTsFile, detailTsContent);

    // DETAIL HTML
    const detailHtmlContent = \`<div class="p-6" *ngIf="data">
  <div class="flex justify-between items-center mb-6">
...
\`;
    fs.writeFileSync(detailHtmlFile, detailHtmlContent);
});

console.log('Moderator Client Frontend Generated Successfully.');
